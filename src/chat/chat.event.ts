...
import { ClientOnEventType, ServerOnEventType } from 'src/utils/constants';

@Injectable()
export class ChatEvent implements ChatHandler {
  constructor(private connectManager: ConnectManagerService) {}

  @OnEvent(ServerOnEventType.CHAT_WITH_SOMEONE)
  talkTosomeone(data: TChatEventGetData): void {
    ...
    client.to(toId).emit(ClientOnEventType.CLIENT_GET_CHAT_DATA, toData);
  }
